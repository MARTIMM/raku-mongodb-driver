...
Create a database object. Can be called directly although not often, e.g.

  my MongoDB::Database $d .= new( $client, 'my_db');

When read-concern is absent it is taken from $client.
...
When read-concern is absent it is taken from its $database.
