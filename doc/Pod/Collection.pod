...
Creates a new Collection object. However, it is easier to call collection on
the database object as shown in the examples above.
...
B<<(field => 0,)>>. When 1 it is included, when 0 it will be excluded. The _id
field is always included unless explicitly excluded like B<<(_id => 0,)>>. The
method returns a MongoDB::Cursor.

=for item1
B<:number-to-skip> is used to skip a number of documents. By defaultt 0 meaning
all documents of the found set is to be returned.

=for item1
B<:number-to-return> is used to ask for a specific number of documents. By defaultt 0 meaning
that none of the documents from the found set should be skipped first.
