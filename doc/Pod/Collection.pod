...
=head1 Synopsis
  ...
  #
  $cursor = $collection.find( (nick => 'camelia'), :$number-to-return(1));
...
=head2 attribute database

  has $.database;

Get the name of the database of this collection. It is set by
...
=head2 attribute full-collection-name
...
Example
  ...
  my MongoDB:Collection $collection .= new( $database, 'perl_users');
...
Find documents in the database. When C<$criteria> is empty all documents are
returned, There are 2 options and a flag to control the search. $projection is
used to select the fields to be returned. It looks like B<<(field => 1,)>> or
B<<(field => 0,)>>. When 1 the field is included, when 0 it will be excluded.
The _id field is always included unless explicitly excluded like B<<(_id => 0,)>>.
The method returns a C<MongoDB::Cursor>.

=for item1
B<:number-to-skip> is used to skip a number of documents. By default 0 meaning
all documents of the found set are to be returned.

=for item1
B<:number-to-return> is used to ask for a specific number of documents. By
default 0 meaning that none of the documents from the found set should be
skipped first.


=begin item1

C<:flags>. This ia a bit variable which can be set using the mask values below.
An example;

  my $c = $collection.find(
   :flags(MongoDB::C-QF-SLAVEOK +| MongoDB::C-QF-NOCURSORTIMOUT)
  );
  for $c.fetch -> BSON::Document $d {
    last if $d<some-item>:exists and $d<some-item> eq 'some value';
  }
  $c.kill;

=for item2
B<MongoDB::C-QF-TAILABLECURSOR>: corresponds to TailableCursor. Tailable means
...
=for item2
B<MongoDB::C-QF-SLAVEOK>: corresponds to SlaveOk.Allow query of replica slave.
Normally these return an error except for namespace 'local'.

=for item2
B<MongoDB::C-QF-OPLOGREPLAY>: corresponds to OplogReplay. Internal replication
use only - driver should not set.

=for item2
B<MongoDB::C-QF-NOCURSORTIMOUT>: corresponds to NoCursorTimeout. The server
...
=for item2
B<MongoDB::C-QF-AWAITDATA>: corresponds to AwaitData. Use with TailableCursor.
...
=for item2
B<MongoDB::C-QF-EXHAUST>: corresponds to Exhaust. Stream the data down full
...
=for item2
B<MongoDB::C-QF-PORTAIL>: corresponds to Partial. Get partial results from a
