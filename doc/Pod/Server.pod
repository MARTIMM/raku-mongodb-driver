...
=head2 attribute is-master

  has Bool $.is-master = False;

Server is a master server. A background process monitors the mongodb server to
check for server changes. This attribute is set by this process.
...
server object. See also set-max-sockets() below.


=head2 method initial-poll

  method initial-poll ( --> Bool )

First time check on the mongodb server. Called in Client object after the Server
object is created. This process is needed to reject the server on ground of its
information. I<Do not use this method directly!>.


=head2 method monitor-server

  method monitor-server ( )

Server monitoring procedures. This starts a background process which runs
as long as the Server object lives. This method is called from the Client
object when the server is accepted for duty.
I<Do not use this method directly!>.
...
Shutdown mongod server. I<Do not use this method directly!>.
