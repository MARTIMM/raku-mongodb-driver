...
* A **Server** in a topology can never be in any other topology. This means that a **Client** object which started the build of a topology using some URI always end up with the same set of **Server**'s belonging to that topology. The URI's can be different although may not contradict. A replicaset must always have replica info in the URI.

* At the moment, the **Client** object holds info about **Server**'s and topology. So all classes need to have a client object to get to the server they want to use for I/O. This path is too long and creates dependencies on the client. It even creates circular dependencies! It should be possible to have a pool of **Server**'s where the **Server**'s are stored by a key and a set of other data given by the **Client** who created them. When this **ServerPool** is a singleton object, every object can get there for a **Server** to talk to. This selection of a server must be done using a set of rules called read- or write concerns and a key.

* **ServerPool** key generation. This key can not be generated from the given URI because different URI's can build up to the same topology with the same set of servers.
...
* For each server from this uri object
  * Search for server in serverpool and get its topology key
  * Every server from the list must return the same key if found
* Generate topology key if there is none.
* For each server from this uri object
  * Client starts monitor if not running already
  * Create and store server in serverpool using topology key
* Server receives info from monitor to set its status
* Client receives new servers info from server
* For each new server from this info
  * Create and store server in serverpool using same topology key

When 2 or more clients process a uri in parallel, wich have as a result, the same set of servers, it might be very possible that servers are inserted with different keys.
* Access to the pool must be controlled by using semaphores.
* The algoritm needs 2 keys. One from the client and one generated by the pool.
  * Client adds a server providing server and clientkey.
  * When not stored yet;
    * Check if client has stored other servers
    * If so, take poolkey from one of the entries. Should be the same for all entries.
    * if not, generate new poolkey
    * Store server with client key and poolkey.
  * When server was stored before, it should be duplicated because each client must be able to add and remove a server.
    * Get the poolkey from that entry.
    * Modify the poolkey of all entries having the same client key

|ck |s  |pk  |note                                                         |
|---|---|----|-------------------------------------------------------------|
|c1 |s1 |p1  |new entry, new pool key
|c1 |s1 |p1  |no changes, same client and server
|c2 |s1 |p1  |no changes, client not substituted. previous poolkey for a c2 entry does not differ -> no further changes.
|c2 |s2 |p2  |new entry, client not saved elsewhere new pool key
|c1 |s3 |p1  |from previous c1 entries with client key c1
|c1 |s2 |p2  |previous poolkey for c1 entry differs therfore all c1 entries change p1 to p2.
|c2 |s3 |p2  |previously p1 but is changed by previous step

<!-- ??
* The method which adds a server returns the topology key of the inserted server provided by the caller, or returns the key of the entry of the server if it already exists in a pool. This key must be taken over by the caller when inserting the next server.
-->
