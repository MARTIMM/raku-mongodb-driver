...
The flag B<:continue-on-error> can be set to let the insert continue its job
...
my %d1 = k1 => 'v1', k2 => 'v2';
my $d2 = %(k1 => 'v1a');
$collection.insert( :continue-on-error, {%d1}, $d2);
...
B<:number-to-skip> is used to skip a number of documents.

B<:number-to-return> is used to ask for a specific number of documents.

B<:no-cursor-timeout> The server normally times out idle cursors after an
...
method update ( %selector, %update,
                Bool :$upsert = False, Bool :$multi-update = False
              ) {...}
=end head2 
...
B<:multi-update> If set, the database will update all matching objects in the
...
B<:single-remove> If set, the database will remove only the first matching
