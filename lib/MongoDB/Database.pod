...
might be more convenient. run-command must have the command at the first key
value pair. Hashes cannot be used because it will not keep the same order for
keys as they will be entered.

First method to insert a document using a C<BSON::Document> See also L<here|https://docs.mongodb.org/manual/reference/command/insert/>
...
whith a colon ':' e.g. B<<$some-array.push: ($some-key => $some-value);>>. This
