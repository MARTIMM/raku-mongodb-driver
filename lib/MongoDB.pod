...
the severity level in the exception object is higher then this threshold. The
default is initialized to C<MongoDB::Severity::Fatal>.
...
Exceptions are logged by C<log> method in the C<Logging> role if the severity
level in the exception object is higher then this threshold. The default is
initialized to C<MongoDB::Severity::Info>.
...
C<$.status> attribute. A part of the data in this object is often set from data
...
or log and maybe die using the examples shown above:
...
server there is also a code. This code will be used here.
...
=head3 collection-name

Collection name involved
