...
method when the server did not play a role in the action.

Some of the fields can be undefined. E.g. when an error occurs while dropping a
database, the collection name would not be set for that operation. When you
think that you can not handle the exception you can throw the exception
yourself like so:
